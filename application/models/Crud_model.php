...
class Crud_model extends CI_Model 
{
    ...
    // Below is unrelated functions but could be useful later
    ...
    function post_edit($id) {
        ...
        $data['edited_date']     =   date('d/m/Y');
        $data['edited_by']     =   $this->session->userdata('author_id');
        $data['status']       =   $this->input->post('status');
        $this->db->where('id' , $id);
        $this->db->update('posts' , $data);
        // move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/post_image/' . $id . '.jpg');
    }
    function member_post_edit($id) {
        $data['title_eng']         =   $this->input->post('title_eng');
        $data['title_kin']     =   $this->input->post('title_kin');
        $data['post_eng']        =   $this->input->post('post_eng');
        $data['post_kin']        =   $this->input->post('post_kin');
    ...
    // function get_subject_filter() {
    //     $query = $this->db->get_where('subject', array('year' => $running_year));
    //     return $query->result_array();
    // }
    ...
    $query1 = $this->db->get('teacher')->result_array();
...
curl_setopt_array($curl, array(
 ...
 CURLOPT_POSTFIELDS => array('action' => 'send-sms','to' => $teacherphone,'from' => 'EAB','sms' => $message,'api_key'=>'dmlBPXZGRmRPYk1qc3B6cEZ2enQ='),
...
// parents
//     $query1 = $this->db->get('student')->result_array();

//         foreach ($query1 as $row) {
//              $teacherphone = $row['parentphone'];
//              $message = $this->input->post('description');
...
//  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
//  CURLOPT_CUSTOMREQUEST => "POST",
//  CURLOPT_POSTFIELDS => array('action' => 'send-sms','to' => $teacherphone,'from' => 'EAB','sms' => $message,'api_key'=>'a0dKQnlEdnVHbm9xQ3FPS2tyRUU='),
...
// }
// accountants
    $query1 = $this->db->get('accountant')->result_array();

        foreach ($query1 as $row) {
             $accountantphone = $row['phone'];
             $message = $this->input->post('description');
...
curl_setopt_array($curl, array(
 ...
 CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
 CURLOPT_CUSTOMREQUEST => "POST",
 CURLOPT_POSTFIELDS => array('action' => 'send-sms','to' => $accountantphone,'from' => 'EAB','sms' => $message,'api_key'=>'dmlBPXZGRmRPYk1qc3B6cEZ2enQ='),
 ...
 echo "cURL Error #:" . $err;
    ...
    function send_new_private_message() {
        ...
        if ($num1 == 0 && $num2 == 0) {
            ...
            $data_message_thread['last_message_timestamp']            = date('d/m/Y');
